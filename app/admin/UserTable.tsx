  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (deleteModal.userId) {
       ...
       const forgetPasswordDocs = await client.fetch(
      `*[_type == "forgetPassword" && userEmail == $email]{_id}`,
      { email: deleteModal.userEmail }
    );

    if (forgetPasswordDocs.length > 0) {
      for (const doc of forgetPasswordDocs) {
      ...
      // Update users list
      setUsers((prev) =>
        prev.filter((user) => user.email !== deleteModal.userEmail)
      );
